...
class RGBXTransformer2(nn.Module):
    ...
    def init_weights(self, pretrained=None, pretrained1=None):
        if isinstance(pretrained, str):
            if isinstance(pretrained1, str):
                load_dualpath_model(self, pretrained, pretrained1)
        else:
...
def load_dualpath_model(model, model_file, model_file2):
    # load raw state_dict
    t_start = time.time()
    ...
    # 加载第二个模型文件
    if isinstance(model_file2, str):
        raw_state_dict2 = torch.load(model_file2, map_location=torch.device('cpu'))
        if 'model' in raw_state_dict2.keys():
            raw_state_dict2 = raw_state_dict2['model']
    else:
        raw_state_dict2 = model_file2

    
    state_dict = {}
    for k, v in raw_state_dict.items():
        if k.find('backbone.patch_embed') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.patch_embed', 'patch_embed')] = v
        elif k.find('backbone.block') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.block', 'block')] = v
        elif k.find('backbone.norm') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.norm', 'norm')] = v
        # elif k.find('decode_head') >= 0:
        #     state_dict[k] = v
    # breakpoint()
    # for k, v in raw_state_dict2.items():
...
class single_mit_b0(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b0, self).__init__(
            patch_size=4, embed_dims=[32, 64, 160, 256], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)


class single_mit_b1(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b1, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)


class single_mit_b2(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b2, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)


class single_mit_b3(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b3, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)


class single_mit_b4(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b4, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 8, 27, 3], sr_ratios=[8, 4, 2, 1],
            drop_rate=0.0, drop_path_rate=0.1)


class single_mit_b5(RGBXTransformer2):
    def __init__(self, fuse_cfg=None, **kwargs):
        super(single_mit_b5, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[4, 4, 4, 4],
