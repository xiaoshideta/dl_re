...
def load_dualpath_model(model, model_file, model_file2):
    ...
    for k, v in raw_state_dict2.items():
        if k.find('backbone.patch_embed') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.patch_embed', 'extra_patch_embed')] = v
        elif k.find('backbone.block') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.block', 'extra_block')] = v
        elif k.find('backbone.norm') >= 0:
            # state_dict[k] = v
            state_dict[k.replace('backbone.norm', 'extra_norm')] = v
    # breakpoint()
