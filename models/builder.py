...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d, load=True, decode_init=1):
        ...
        if self.load:
            print(self.load)
            self.init_weights(cfg, pretrained=cfg.pretrained_model, pretrained1=cfg.pretrained_model1, decode_init=decode_init)
...
class EncoderDecoder2(nn.Module):
    ...
    def init_weights(self, cfg, pretrained=None, pretrained1=None, decode_init=0):
        if pretrained:
            logger.info('Loading pretrained model: {}'.format(pretrained))
            if pretrained1:
                logger.info('Loading pretrained model: {}'.format(pretrained1))
