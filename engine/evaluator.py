...
class Evaluator(object):
    ...
    def run(self, model_path, model_indice, log_file, log_file_link, modell, flag):
        ...
        if modell is not None: 
            ...
            print("val start!")
            # 加载模型参数
            self.val_func = modell
            if len(self.devices) == 1:
                result_line, mIoU = self.single_process_evalutation(flag)
            else:
                result_line, mIoU = self.multi_process_evaluation(flag)
    ...
    def single_process_evalutation(self, flag):
        ...
        for idx in tqdm(range(self.ndata)):
            dd = self.dataset[idx]
            # 转到evalutor的函数定义
            results_dict = self.func_per_iteration(dd,self.devices[0],flag)
    ...
    def sliding_eval_rgbX(self, img, modal_x, crop_size, stride_rate, device=None):
        ...
        for s in self.multi_scales:
            ...
            if modal_x is not None:
                ...
                processed_pred += self.scale_process_rgbX(img_scale, modal_x, (ori_rows, ori_cols),
                                                        crop_size, stride_rate, device)
